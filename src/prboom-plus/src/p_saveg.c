...
static int number_of_mobj_thinkers;
...
void P_ThinkerToIndex(void)
{
  ...
  number_of_mobj_thinkers = 0;
  int number_of_thinkers = 0;
  for (th = thinkercap.next ; th != &thinkercap ; th=th->next)
    {
      if (th->function == P_RemoveThinkerDelayed)
        continue;
      th->prev = (thinker_t *) ++number_of_thinkers;  // 0 reserved for NULL
      if (th->function == P_MobjThinker)
        ++number_of_mobj_thinkers;
    }
}
...
void P_ArchiveThinkers (void)
{
   ...
   * - killough 2/14/98
   * cph - use number_of_mobj_thinkers saved by P_ThinkerToIndex above
   ...
   */
  CheckSaveGame(number_of_mobj_thinkers*(sizeof(mobj_t)-3*sizeof(fixed_t)+4+3*sizeof(void*)) +1);
...
// P_UnArchiveThinker
//
static mobj_t* P_UnArchiveThinker (void)
{
  byte tclass = *save_p++;
  if (tclass != tc_mobj)
    I_Error("P_UnarchiveThinker: Unknown tclass %i in savegame", tclass);
...
// P_UnArchiveSpecial
//
static void P_UnArchiveSpecial (void)
{
  byte tclass = *save_p++;
  ...
  switch (tclass)
    ...
    default:
      I_Error("P_UnarchiveSpecial: Unknown tclass %i in savegame", tclass);
    }
}
...
void P_UnarchiveThinkersAndSpecials(void)
{
  ...
  byte      **map_p;     // index to save_p
  byte      *isMobj;     // is index mobj
  byte      *mobjs_p;    // start of mobjs
  byte      *targets_p;  // start of targets
  byte      *specials_p; // start of specials
  byte      *end_p;
  ...
  mobjs_p = save_p;
  {
    ...
    if (*--save_p != tc_end)
      I_Error ("P_UnarchiveThinkersAndSpecials: Unknown tclass %i in savegame. Expected tc_end (%i).", *save_p, tc_end);
  ...
  specials_p = save_p;
  {
    ...
    while ((tclass = *save_p++) != tc_endspecials)
      switch (tclass)
        ...
        default:
          I_Error("P_UnarchiveThinkersAndSpecials: Something is fucky");
        }

    end_p = save_p;
  } // count specials

  mobj_p = calloc(size, sizeof *mobj_p);
  map_p = calloc(size, sizeof *map_p);
  isMobj = calloc(size, sizeof *isMobj);

  // map mobjs
  save_p = mobjs_p;
  {
    while (*save_p++ == tc_mobj)
      {
        byte *start = save_p - 1;
        PADSAVEP();
        map_p[(size_t)((mobj_t*)save_p)->thinker.prev] = start;
        isMobj[(size_t)((mobj_t*)save_p)->thinker.prev] = 1;
        save_p += sizeof(mobj_t);
      }
  }
    ...
    while ((tclass = *save_p++) != tc_endspecials)
      {
        ...
        switch (tclass)
          ...
          case tc_ceiling:
            PADSAVEP();
            {
              map_p[(size_t)((ceiling_t*)save_p)->thinker.prev] = start;
              save_p += sizeof(ceiling_t);
              break;
            }
          ...
          case tc_door:
            PADSAVEP();
            {
              map_p[(size_t)((vldoor_t*)save_p)->thinker.prev] = start;
              save_p += sizeof(vldoor_t);
              break;
            }
          ...
          case tc_floor:
            PADSAVEP();
            {
              map_p[(size_t)((floormove_t*)save_p)->thinker.prev] = start;
              save_p += sizeof(floormove_t);
              break;
            }
          ...
          case tc_plat:
            PADSAVEP();
            {
              map_p[(size_t)((plat_t*)save_p)->thinker.prev] = start;
              save_p += sizeof(plat_t);
              break;
            }
          ...
          case tc_flash:
            PADSAVEP();
            {
              map_p[(size_t)((lightflash_t*)save_p)->thinker.prev] = start;
              save_p += sizeof(lightflash_t);
              break;
            }
          ...
          case tc_strobe:
            PADSAVEP();
            {
              map_p[(size_t)((strobe_t*)save_p)->thinker.prev] = start;
              save_p += sizeof(strobe_t);
              break;
            }
          ...
          case tc_glow:
            PADSAVEP();
            {
              map_p[(size_t)((glow_t*)save_p)->thinker.prev] = start;
              save_p += sizeof(glow_t);
              break;
            }
          ...
          case tc_flicker:
            PADSAVEP();
            {
              map_p[(size_t)((fireflicker_t*)save_p)->thinker.prev] = start;
              save_p += sizeof(fireflicker_t);
              break;
            }
          ...
          case tc_elevator:
            PADSAVEP();
            {
              map_p[(size_t)((elevator_t*)save_p)->thinker.prev] = start;
              save_p += sizeof(elevator_t);
              break;
            }
          ...
          case tc_scroll:
            {
              map_p[(size_t)((scroll_t*)save_p)->thinker.prev] = start;
              save_p += sizeof(scroll_t);
              break;
            }
          ...
          case tc_pusher:
            {
              map_p[(size_t)((pusher_t*)save_p)->thinker.prev] = start;
              save_p += sizeof(pusher_t);
              break;
            }

          default:
            I_Error("P_UnarchiveThinkersAndSpecials: Something is really fucky");
          }
      }
  } // map specials
